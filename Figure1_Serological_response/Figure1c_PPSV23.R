# Figure1c - PPSV23 vaccine responsiveness metrics
...
PPSV23 <- read_excel("Figure1c.xls", sheet = "PPSV23_data")
PPSV23_titer <- FormatTiters(PPSV23)
PPSV23_endpoints <- Calculate_maxRBA(PPSV23_titer, method = "exp", scoreFun = "sum")
PPSV23_maxRBA_rank <- PPSV23_endpoints$maxRBA %>% dense_rank


# Strength, Extent and Rank calculation

# load the data
Data_pre <- read_excel(path="Figure1c.xls", sheet = "PPSV23_Pre") %>% 
  as.data.frame
Data_post <- read_excel(path="Figure1c.xls", sheet = "PPSV23_Post") %>% 
  as.data.frame
...
#load the meta data
meta <- read_excel(path = "Figure1c.xls", sheet = "PPSV23_Meta") %>% 
  as.data.frame
...
new.meta <- cbind(meta, 
        ...
        sumRBA = PPSV23_maxRBA_rank)


write.csv(new.meta, "PPSV23_Meta_antibody_ranked_old.csv")
...
pdf(file = "Figure1c_Bubble_plot.pdf", width = 10, height = 10)
bubble_plot
dev.off()
...
pdf(file = "Figure1c_Ranking_scores.pdf", width = 10, height = 12)
standalone_scores
dev.off()
