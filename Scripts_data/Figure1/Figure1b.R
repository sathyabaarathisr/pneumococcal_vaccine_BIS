...
# maxRBA
PCV13 <- read.csv(file="PCV13_data.csv", header=TRUE, sep=",")
...
Data_pre <- read_excel("PCV13_data.xls", sheet = "PCV13_Pre")
rownames(Data_pre) <- Data_pre$DonorID
Data_pre <- Data_pre[,-1]

Data_post <- read_excel("PCV13_data.xls", sheet = "PCV13_Post")
...
#load the meta data
meta <- read_excel(path = "PCV13_data.xls", sheet = "PCV13_Meta") %>% 
  as.data.frame
...
write.csv(new.meta, "Meta_antibody_ranked_old.csv")
serotypes <- colnames(Data_post_processed)[1:length(Data_post_processed)]


# Bubble plot showing fold difference in antibody titers post and pre immunization
# for each strain and for each individual.

pre.color <- "grey1"
pst.color <- "midnightblue"
...
#### Bubble plot for FC sum, FC_rank, FC extent, Z-score and cumulative rank
...
Sex_Mark <- ggplot(ranked.list,
                      aes(x="Sex", y = DonorID, color = Gender, label = sprintf(Gender))) + 
  #geom_point(size = 10) + 
...
pdf(file = "Ranking_scores.pdf", width = 10, height = 12)
standalone_scores
dev.off()
